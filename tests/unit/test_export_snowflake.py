...
# Fixed, trusted directory that holds this test module's JSON fixtures.
# Resolving it once, absolutely, keeps the path fully under this module's
# control (CWE-73 / path-manipulation remediation).
_RESOURCES_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'resources')


def _resource_path(filename):
    """Return the absolute path to a fixture inside the trusted resources dir.

    Only the basename of ``filename`` is honoured, and the result is verified
    to stay inside ``_RESOURCES_DIR`` so a caller can never traverse outside
    the intended fixture directory.
    """
    safe_name = os.path.basename(filename)
    resolved = os.path.normpath(os.path.join(_RESOURCES_DIR, safe_name))
    if os.path.dirname(resolved) != _RESOURCES_DIR:
        raise ValueError(f'Invalid resource file name: {filename!r}')
    return resolved
